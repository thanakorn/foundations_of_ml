...
# %% RBF Improvement
# Normalized features
normalized_diabetes_data = diabetes_data
for i in range(num_features):
    feature = diabetes_data[:,i]
    normalized_feature = scale(feature, axis = 0)
    # print('--------')
    # print('Mean : ', normalized_feature.mean())
    # print('SD : ', normalized_feature.std())
...
for train_indices, test_indices in kfold.split(normalized_diabetes_data):
    ...
    ax[0].set_xlabel('Actual')
    ax[0].set_ylabel('Predict')
    ax[0].text(0.75, 0.1, f'Mean Sqr Error = %.2f' % mean_squared_error(test_target, predict_lr), weight = 'bold', horizontalalignment='center',verticalalignment='center', transform=ax[0].transAxes)
    ax[0].scatter(test_target, predict_lr)
    ax[1].grid(True)
    ax[1].set_title('RBF')
    ax[1].set_xlabel('Actual')
    ax[1].set_ylabel('Predict')
    ax[1].text(0.75, 0.1, f'Mean Sqr Error = %.2f' % mean_squared_error(test_target, predict_rbf), weight = 'bold', horizontalalignment='center',verticalalignment='center', transform=ax[1].transAxes)
...
#     predict_rbf = U_test @ w_rbf
#     predict_svr = svr_rbf.predict(test_data)
#     fig, ax = plt.subplots(nrows=1, ncols=3, figsize = (18,3))
#     ax[0].grid(True)
#     ax[0].set_title('RBF')
#     ax[0].set_xlabel('Actual')
#     ax[0].set_ylabel('Predict')
#     ax[0].text(0.75, 0.1, f'Mean Sqr Error = %.2f' % mean_squared_error(test_target, predict_rbf), weight = 'bold', horizontalalignment='center',verticalalignment='center', transform=ax[0].transAxes)
#     ax[0].scatter(test_target, predict_rbf)
#     ax[1].grid(True)
#     ax[1].set_title('scikit-learn')
#     ax[1].set_xlabel('Actual')
#     ax[1].set_ylabel('Predict')
#     ax[1].text(0.75, 0.1, f'Mean Sqr Error = %.2f' % mean_squared_error(test_target, predict_svr), weight = 'bold', horizontalalignment='center',verticalalignment='center', transform=ax[1].transAxes)
#     ax[1].scatter(test_target, predict_svr, color='r')
#     ax[2].grid(True)
#     ax[2].set_title('Result Distributions')
#     ax[2].boxplot([predict_rbf, predict_svr], labels=['RBF', 'sklearn'])

train_data, test_data, train_target, test_target = train_test_split(diabetes_data, diabetes_target, test_size=0.3)
svr_rbf = SVR(kernel='rbf', gamma='scale')
svr_rbf.fit(train_data, train_target)
predict_svr = svr_rbf.predict(train_data)
fig, ax = plt.subplots(figsize = (3,3))
ax.set_ylim((0,300))
ax.scatter(train_target, predict_svr)
